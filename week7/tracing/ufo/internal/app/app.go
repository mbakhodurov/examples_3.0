...
import (
	...
	"github.com/mbakhodurov/homeworks2/week7/tracing/platform/pkg/grpc/health"
	"github.com/mbakhodurov/homeworks2/week7/tracing/platform/pkg/logger"
...
// initTracing инициализирует OpenTelemetry трейсер
func (a *App) initTracing(ctx context.Context) {
	shutdownTracer, err := tracing.InitTracer(ctx, config.AppConfig().Tracing.TracingConfig())
	if err != nil {
		slog.Error("не удалось инициализировать трейсер", "error", err)
		os.Exit(1)
	}

	closer.Add("трейсер", shutdownTracer)
}
...
func (a *App) initGRPCServer(ctx context.Context) {
	...
	ufov1.RegisterUFOServiceServer(a.grpcServer, api)
}
