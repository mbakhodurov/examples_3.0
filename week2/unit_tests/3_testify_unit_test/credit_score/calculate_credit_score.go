...
var (
	...
	ErrInvalidSalary = errors.New("некорректная зарплата")
)
...
func CalculateCreditScore(client Client) (int, error) {
	if client.Age < 0 {
		return 0, fmt.Errorf("%w: %d", ErrInvalidAge, client.Age)
	}
	...
	return max(0, min(1000, score)), nil
}

func checkSalary(client Client) error {
	if client.AverageSalary < 0 {
		return fmt.Errorf("%w: %.2f", ErrInvalidSalary, client.AverageSalary)
	}
